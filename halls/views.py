...
def explore(request):
    recent_halls = Hall.objects.all().order_by('-id')[:3]
    popular_halls  = [ Hall.objects.get(pk=1) ]
    return render(request, 'halls/explore.html',{'recent_halls':recent_halls, 'popular_halls':popular_halls })


@login_required
def recent_view(request):
    recent_searched = Hall.objects.get(pk=1)
    return render(request, 'halls/hall_ad_video.html', {'recent_searched':recent_searched})
...
def Add_video(request, pk):
    ...
    return render(request, "halls/hall_add_video.html", { "form" : form, "search_form":search_form , 'hall':hall })
...
class DeleteVideo(LoginRequiredMixin, generic.DeleteView):
    ...
    def ge_object(self):
        ...
        if not video.hall.super == self.request.user:
            raise Http404
        return video
...
class UpdateHall(LoginRequiredMixin, generic.UpdateView):
    ...
    def ge_object(self):
        ...
        if not hall.super == self.request.user:
            raise Http404
        return hall
...
def handler404(request, exception):
    ...
    return render(request, "halls/error_404.html", context, status=404)
